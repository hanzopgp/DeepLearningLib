...
class LeakyReLU(nndiy.core.Activation):
	...
	def backward(self):
		self._grad_output = np.where(self._input > 0, 1, self.ALPHA) * self._delta
...
class Identity(nndiy.core.Activation):
	...
	def backward(self):
		self._grad_output = np.ones_like(self._input) * self._delta
...
class ReLU(nndiy.core.Activation):
	...
	def backward(self):
		self._grad_output = np.where(self._input > 0, 1, 0) * self._delta
...
class Sigmoid(nndiy.core.Activation):
	...
	def backward(self):
		...
		self._grad_output = sigmoid * (1 - sigmoid) * self._delta
...
class Softmax(nndiy.core.Activation):
	...
	def backward(self):
		...
		self._grad_output = self._delta * (soft*(1-soft))
...
class Tanh(nndiy.core.Activation):
	...
	def backward(self):
		self._grad_output = (1 - np.tanh(self._input)**2) * self._delta
